...
locals {
  # Region flows in from the connected credential.
  ...
  # Step 11: the new storage_class param, surfaced in the resource name so it
  # is visible in the UI without changing the workshop-s3-bucket contract.
  label    = "${local.bucket_name} (${var.storage_class}, ${var.retention_days}d retention)"
